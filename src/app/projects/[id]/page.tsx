...
export default async function ProjectDetailPage({
  ...
  const grants = accessGrants.filter(
    (g) => g.targetType === "project" && g.targetId === project.id,
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <Field
          ...
          value={project.deliveryModel?.replace("_", "-")}
        />
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {owners.length === 0 && (
            <span className="text-xs text-text-faint">No owner recorded.</span>
          )}
        </div>
      </div>
        ...
        <div className="space-y-1.5">
          {grants.map((g) => {
            ...
            return (
              <div
                key={g.id}
                className="flex items-center justify-between rounded-md border border-border-soft bg-panel/60 px-3 py-2 text-xs"
              >
                <span className="text-text-muted">
                  {person?.name} — <span className="font-mono">{g.level}</span>
                </span>
                {g.vaultReference && (
          ...
          {grants.length === 0 && (
            <p className="text-xs text-text-faint">
              No explicit grants recorded — visibility follows default branch
              role rules.
            </p>
          )}
        </div>
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {project.syncHistory.map((s, i) => (
            <div
              key={i}
              className="flex items-center justify-between rounded-md border border-border-soft bg-panel/60 px-3 py-2 text-xs font-mono"
            >
              <span className="text-info">{sourceLabel(s.source)}</span>
              <span className="text-text-muted">{s.accountLabel}</span>
              <span
                className={s.reachable === false ? "text-danger" : "text-live"}
              >
          ...
          {project.syncHistory.length === 0 && (
            <p className="text-xs text-text-faint">
              Never auto-synced — manual entry only.
            </p>
          )}
        </div>
      </div>
      ...
      {value ? (
        link ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className={`mt-0.5 block truncate text-sm text-live hover:underline ${mono ? "font-mono" : ""}`}
          >
            {value}
          </a>
        ) : (
          <div
            className={`mt-0.5 truncate text-sm text-text ${mono ? "font-mono" : ""}`}
          >
            {value}
          </div>
        )
